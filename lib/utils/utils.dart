...
class Utils{
  ...
  static String getGenres(Object item, MediaType mediaType) {
    ...
    if(mediaType == MediaType.actor){
      ...
      TvSeries tvSeries = item as TvSeries;
      genres = tvSeries.genreIds;
      }
      List<String> genresListStr = [];
      genres.forEach((id){
        [
          {28: 'Action'},
          {12: 'Adventure'},
  ...
  static String getDate(Object object, MediaType mediaType){
    switch(mediaType){
      ...
      case (MediaType.actor):
        ...
        if(actor.deathDay != ''){
          date = '$date - ${actor.deathDay}';
        }
        return date;
    }
  }

  static String getOriginCountry(TvSeries tvSeries){
    String countryCode = tvSeries.originCountry[0];
    String countryName = 'N/A';
    countryCodesMap.forEach((keyPair) {
      if(keyPair["code"] == countryCode) {
        countryName = keyPair["name"].toString();
        return;
      }
    });
    List<String> split = countryName.split(',');
    countryName = split.reversed.join(' ');
    return countryName;
  }
